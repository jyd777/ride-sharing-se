...
(function(vue) {
  ...
  const _sfc_main$h = {
    ...
    methods: {
      ...
      async generateGroupAvatar(groupId, members) {
        ...
        const avatars = members.slice(0, 4).map((m2) => m2.avatar);
        if (avatars.length === 1) {
      ...
      drawAvatar(ctx, avatarUrl, x, y2, width, height) {
        return new Promise((resolve) => {
          uni.getImageInfo({
            ...
            success: (res) => {
              ...
              ctx.closePath();
              ctx.clip();
              ctx.drawImage(res.path, x, y2, width, height);
              ctx.restore();
              resolve();
            },
            fail: () => {
              ctx.save();
              ctx.beginPath();
              ctx.arc(x + width / 2, y2 + height / 2, Math.min(width, height) / 2, 0, Math.PI * 2);
              ctx.closePath();
              ctx.clip();
    ...
    methods: {
      ...
      uploadAvatar(event) {
        ...
        if (file) {
          ...
          reader.onload = (e2) => {
            this.user.avatar = e2.target.result;
          };
  ...
  const _sfc_main$2 = {
    ...
    methods: {
      ...
      onMapTap(e2) {
        formatAppLog("log", "at pages/index/home.vue:174", "地图被点击了", e2);
      },
      onMarkerTap(e2) {
        formatAppLog("log", "at pages/index/home.vue:177", "标记被点击了", e2);
      },
      onRegionChange(e2) {
        formatAppLog("log", "at pages/index/home.vue:180", "地图区域改变", e2);
      },
    ...
    computed: {
      filteredOrders() {
        ...
        if (sortOption === "time-asc" || sortOption === "time-desc") {
          ...
          filtered.sort((a2, b2) => {
            const timeA = parseChineseDate(a2.date);
            const timeB = parseChineseDate(b2.date);
            return sortOption === "time-asc" ? timeA - timeB : timeB - timeA;
          });
        } else if (sortOption === "price-asc") {
          filtered.sort((a2, b2) => a2.price - b2.price);
        } else if (sortOption === "price-desc") {
          filtered.sort((a2, b2) => b2.price - a2.price);
        }
        return filtered;
      }
    },
    methods: {
      onOrderTypeChange(e2) {
        this.orderTypeIndex = e2.detail.value;
      },
      // 新增状态筛选方法
      onStatusChange(e2) {
        this.statusIndex = e2.detail.value;
      ...
      onDayChange(e2) {
        this.dayIndex = e2.detail.value;
      },
      onSortChange(e2) {
        this.sortIndex = e2.detail.value;
      },
      applyToJoin(orderId) {
      },
      // 新增方法：获取状态对应的文本
